...
class GzipResponse
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Never compress Livewire or Filament AJAX — they send/receive JSON and
        // handle their own response parsing. Compressing these causes 405 errors
        // when Livewire can't parse the response and retries incorrectly.
        if (
            $request->is('livewire/*') ||
            $request->header('X-Livewire') ||
            $request->header('X-Filament-Component')
        ) {
            return $response;
        }
