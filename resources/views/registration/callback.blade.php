...
@section('title', 'Confirming Payment — ' . setting('camp_name', 'Ogun Youth Camp'))
    ...
    <div class="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div class="max-w-md w-full text-center" x-data="callbackPage()" x-init="poll()">
            ...
            <div x-show="status === 'checking'">
                ...
                <p class="text-gray-500 text-sm">Please wait while we verify your payment with Paystack…</p>
            ...
            <div x-show="status === 'active'" class="space-y-4">
                ...
                <div class="bg-gray-100 rounded-xl p-4 font-mono text-2xl font-bold text-navy tracking-widest">
                    <span x-text="code"></span>
                </div>

                <div class="bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-sm text-yellow-800">
                    ⚠ <strong>Save this code.</strong> You will need it to complete your registration.
                    A copy has also been sent to your phone via SMS.
                </div>

                <a :href="registerUrl"
                   class="block w-full bg-navy text-white font-bold py-3 rounded-xl hover:bg-steel transition">
                    Complete Registration →
                </a>
            </div>

            {{-- Timeout / still pending state --}}
            <div x-show="status === 'pending'" class="space-y-4">
                ...
                <p class="text-gray-600 text-sm">
                    ...
                    <strong>Check your phone for an SMS with your registration code.</strong>
                </p>

                <div class="bg-gray-100 rounded-xl p-4 font-mono text-xl font-bold text-navy tracking-widest">
                    <span x-text="code"></span>
                </div>

                <p class="text-xs text-gray-400">
                    Once you receive the SMS, enter the code on the registration page.
                </p>

                <div class="flex gap-3">
                    <button @click="poll()" class="flex-1 border border-navy text-navy font-bold py-3 rounded-xl hover:bg-navy/5 transition text-sm">
                        Check Again
                    </button>
                    <a href="{{ route('registration.index') }}" class="flex-1 bg-navy text-white font-bold py-3 rounded-xl hover:bg-steel transition text-sm text-center">
                        Enter Code Manually
                    </a>
                </div>
            </div>
...
@push('scripts')
    <script>
        function callbackPage() {
            const code       = new URLSearchParams(window.location.search).get('reference') || '';
            const statusUrl  = `/api/v1/payment/status/${code}`;
            const maxPolls   = 10;

            return {
                status:    'checking',
                code:      code,
                registerUrl: `/registration/form/${code}`,
                errorMsg:  '',
                polls:     0,
                ...
                async doPoll() {
                    ...
                    try {
                        ...
                        this.status  = 'error';
                        this.errorMsg = 'Could not reach the server. Please check your internet connection.';
                    }
                }
            }
        }
